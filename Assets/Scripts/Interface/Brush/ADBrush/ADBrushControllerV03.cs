...
namespace VRBrush.Interface.Brush.ADBrush
{
    /// <summary>
    /// ADBrush控制器变体V03
    /// 特点：有Rolling控制（手腕可控制截面旋转），无自定义截面功能（固定使用line_segment.json）
    /// 用于实验1的条件3
    /// </summary>
    public class ADBrushControllerV03 : BaseBrushController
    {
        [Header("ADBrush V03 Settings")]
        ...
        public override string BrushName => "ADBrush V03 (Roll/NoCustom)";
        ...
        // Rolling角度追踪（与原ADBrushController相同的累积方式）
        private float accumulatedRollingAngle = 0f;
        private Vector3 lastTipDirectionProjection = Vector3.zero;
        private bool hasInitializedRolling = false;
        ...
        protected override void Start()
        {
            ...
            if (currentMorphableShape != null)
            {
                ...
                if (universalBrushPreview != null && currentMorphableShape.BaseShape != null)
                {
                    ...
                    universalBrushPreview.SetPreviewColor(new Color(0.9f, 0.9f, 0.5f, 0.7f)); // 黄色标识V03
            ...
            CreateAngleDisplay();
            Debug.Log($"ADBrushControllerV03: Initialized with fixed shape '{FIXED_SHAPE_NAME}'");
        }
        ...
        {
            var previewObj = new GameObject("ADBrushV03_SectionEdgePreview");
            ...
            sectionEdgePreview.Initialize(null, new Color(0.9f, 0.9f, 0.3f, 0.8f), 0.002f); // 黄绿色
        }
        ...
        private void ConfirmPointOnRelease()
        {
            ...
            // V03特点：有Rolling控制，根据拖拽距离决定是否允许rolling
            const float minDragDistanceForRolling = 0.1f;
            Quaternion rotation;
        ...
        {
            angleDisplayObject = new GameObject("ADBrushV03_AngleDisplay");
        ...
        /// <summary>
        /// V03专用：让通用预览在绘制时使用基于拖拽的朝向，与实际Stroke一致
        /// V03有Rolling控制，所以使用自己的accumulatedRollingAngle字段
        /// </summary>
        protected override Quaternion? GetMainPreviewRotationOverride()
        {
            // 仅在按住扳机拖拽时，用拖拽方向+正确的up来驱动预览朝向
        ...
        private void LoadShape(string shapeName)
        {
            ...
            if (currentMorphableShape == null)
            {
                Debug.LogWarning($"ADBrushControllerV03: Failed to load shape '{shapeName}', using fallback");
        ...
        private void UpdateStrokeMesh(bool forceUpdate = false)
        {
            ...
            if (shapeBuilder.PointCount < 2)
            {
                ...
                {
                    Debug.LogWarning($"ADBrushControllerV03: Not enough points ({shapeBuilder.PointCount}/2 minimum)");
